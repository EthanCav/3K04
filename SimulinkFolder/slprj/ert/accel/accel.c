...
static void accel_SystemCore_setup(freedomk64f_fxos8700_accel_T *obj);
static void accel_SystemCore_setup_n(dsp_simulink_MovingAverage_ac_T *obj);
static void accel_SystemCore_setup(freedomk64f_fxos8700_accel_T *obj)
{
...
/* System initialize for referenced model: 'accel' */
void accel_Init(DW_accel_f_T *localDW)
{
  ...
  localDW->obj_g.SampleTime = accel_P.FXOS87006AxesSensor1_SampleTime;
  accel_SystemCore_setup(&localDW->obj_g);
  ...
  accel_SystemCore_setup_n(&localDW->obj);
...
/* Output and update for referenced model: 'accel' */
void accel(real_T *rty_Outport, B_accel_c_T *localB, DW_accel_f_T *localDW)
{
  g_dsp_internal_SlidingWindowA_T *obj;
  real_T csum;
  real_T modValueRev;
  ...
  if (localDW->obj_g.SampleTime != accel_P.FXOS87006AxesSensor1_SampleTime) {
    localDW->obj_g.SampleTime = accel_P.FXOS87006AxesSensor1_SampleTime;
  }
   ...
   *  MATLABSystem: '<Root>/Moving Average1'
   */
  *rty_Outport = accel_P.Gain_Gain * localB->rtb_MovingAverage1_m;
}

/* Termination for referenced model: 'accel' */
void accel_Term(DW_accel_f_T *localDW)
{
  g_dsp_internal_SlidingWindowA_T *obj;
...
void accel_initialize(const char_T **rt_errorStatus, RT_MODEL_accel_T *const
                      accel_M)
{
  /* Registration code */

  /* initialize error status */
  rtmSetErrorStatusPointer(accel_M, rt_errorStatus);
}
