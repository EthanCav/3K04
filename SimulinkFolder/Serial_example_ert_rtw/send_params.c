 ...
 * C/C++ source code generated on : Mon Nov 21 17:08:47 2022
 *
...
void send_params_Init(void)
{
  ...
  Serial_example_DW.obj_g.isInitialized = 0;
  Serial_example_DW.obj_g.matlabCodegenIsDeleted = false;
  Serial_example_SystemCore_setup(&Serial_example_DW.obj_g);
}
...
void send_params(void)
{
  ...
  while (status != 0) {
    ...
    status = MW_SCI_Transmit(Serial_example_DW.obj_g.MW_SCIHANDLE,
      &TxDataLocChar[0], 9U);
  }
...
void send_params_Term(void)
{
  ...
  if (!Serial_example_DW.obj_g.matlabCodegenIsDeleted) {
    Serial_example_DW.obj_g.matlabCodegenIsDeleted = true;
    if ((Serial_example_DW.obj_g.isInitialized == 1) &&
        Serial_example_DW.obj_g.isSetupComplete) {
      MW_SCI_Close(Serial_example_DW.obj_g.MW_SCIHANDLE);
    }
  }
